...
const cheio = montar();
{
  ...
  ok('8 slides no roteiro', r && r.slides === 8, r && r.slides);
  ok('deck final tem exatamente 8 slides', a.deck._slides.length === 8, a.deck._slides.length);
...
console.log('\n── 5. Logo inacessível (rollback) ──');
{
  ...
  ok('logo quebrado não derruba a geração (ou faz rollback limpo)',
    (!erro && a.deck._slides.length === 8) || (erro && a.deck._slides.length === 3),
